...
\begin{enumerate}
    ...
    \item{
        ...
        \begin{enumerate}
            ...
            \item{
                ...
                Dejando libre la posición $(i, j)$ (es decir, este nuevo piso es idéntico al piso $P$.)
            }
        \end{enumerate}
    }
    \item{
    ...
    \item{
        ...
        \begin{enumerate}
            \item{
                Buscamos la siguiente posición libre $(i', j')$ de la misma manera que en el paso \ref{buscar-posicion-libre}, pero partiendo de $(i, j+1)$ en vez de la esquina superior izquierda, ó $(i+1, 1)$ si $j=n$.
            }
            \item{
...
Si $q_{ij} \in \{ \texttt{SensorVertical}, \texttt{SensorHorizontal}, \texttt{SensorCuadruple} \}$:

\begin{itemize}
    \item{
        Podamos si su señal impacta otro sensor o viceversa; es decir, si $\textsc{Impacta-Otro-Sensor}(q_{ij}) = \textbf{true}$.

        \begin{pseudo}
            \Procedure{Impacta-Otro-Sensor}{$q_{ij}$}
                \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                    \For{$i'$ \In $rango$}
                        \State \textbf{break if} $q_{i'j} = \texttt{Pared}$
                        \If{$\textsc{Sensores-Incompatibles}(q_{ij}, q_{i'j})$}
                ...
                \For{$rango$ \In $\{ (j, \ldots, 1), (j, \ldots, n)  \}$}
                    \For{$j'$ \In $rango$}
                        \State \textbf{break if} $q_{ij'} = \texttt{Pared}$
                        \If{$\textsc{Sensores-Incompatibles}(q_{ij}, q_{ij'})$}
                            \State \Return{\textbf{true}}
                        \EndIf
                    \EndFor
                \EndFor
        ...
        \begin{pseudo}
            \Procedure{Sensores-Incompatibles}{$q_{ij}, q_{i'j'}$}
                \If{$i = i'$}
                    \If{$\textsc{Sensa-Horizontalmente}(q_{ij}) \lor \textsc{Sensa-Horizontalmente}(q_{i'j'})$}
                        \State \Return{\textbf{true}}
                    \EndIf
                \EndIf
                \If{$j = j'$}
                    \If{$\textsc{Sensa-Verticalmente}(q_{ij}) \lor \textsc{Sensa-Verticalmente}(q_{i'j'})$}
                        \State \Return{\textbf{true}}
                    \EndIf
                \EndIf
                \State \Return{\textbf{false}}
        ...
        \begin{pseudo}
            \Procedure{Sensa-Verticalmente}{$q_{ij}$}
                \State \Return{$q_{ij} \in \{ \texttt{SensorVertical}, \texttt{SensorCuadruple} \}$}
            \EndProcedure
        \end{pseudo}

        \begin{pseudo}
            \Procedure{Sensa-Horizontalmente}{$q_{ij}$}
                \State \Return{$q_{ij} \in \{ \texttt{SensorHorizontal}, \texttt{SensorCuadruple} \}$}
            \EndProcedure
        \end{pseudo}
    }
    \item{
        ...
        \begin{pseudo}
            \State $podar \leftarrow \textbf{false}$
            \If{$q_{ij} = \texttt{SensorVertical}$}
                \For{$rango$ \In $\{ (j, \ldots, 1), (j, \ldots, n)  \}$}
                    \For{$j'$ \In $rango$}
                        \State \textbf{break if} $q_{ij'} = \texttt{Pared}$
                        \If{$q_{ij'} = \texttt{Libre} \land
                                \textsc{Hay-Un-Sensor-Horizontal-En-Su-Columna}(q_{ij'})$}
                            \State $podar \leftarrow \textbf{true}$
                            \State \textbf{break}
                        \EndIf
                    \EndFor
                \EndFor
            \EndIf
            \If{$q_{ij} = \texttt{SensorHorizontal}$}
                \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                    \For{$i'$ \In $rango$}
                        \State \textbf{break if} $q_{i'j} = \texttt{Pared}$
                        \If{$q_{i'j} = \texttt{Libre} \land
                                \textsc{Hay-Un-Sensor-Vertical-En-Su-Fila}(q_{i'j})$}
                            \State $podar \leftarrow \textbf{true}$
                            \State \textbf{break}
                        \EndIf
                    \EndFor
                \EndFor
            \EndIf
            \State Podamos $Q$ si $podar = \textbf{true}$.
        \end{pseudo}

        \begin{pseudo}
            \Procedure{Hay-Un-Sensor-Horizontal-En-Su-Columna}{$q_{ij}$}
                \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                    \For{$i'$ \In $rango$}
                        \State \textbf{break if} $q_{i'j} = \texttt{Pared}$
                        \If{$q_{i'j} = \texttt{SensorHorizontal}$}
                            \State \Return{\textbf{true}}
                        \EndIf
                    \EndFor
                \EndFor
        ...
        \begin{pseudo}
            \Procedure{Hay-Un-Sensor-Vertical-En-Su-Fila}{$q_{ij}$}
                \For{$rango$ \In $\{ (j, \ldots, 1), (j, \ldots, n)  \}$}
                    \For{$j'$ \In $rango$}
                        \State \textbf{break if} $q_{ij'} = \texttt{Pared}$
                        \If{$q_{ij'} = \texttt{SensorVertical}$}
    ...
    \item{
        Podamos si el costo del piso supera o iguala el de la mejor solución $S$ hallada hasta el momento; es decir, si $\textsc{Costo}(Q) \geq \textsc{Costo}(S)$.
    }
\end{itemize}

Si $q_{ij} = \texttt{Libre}$:

\begin{itemize}
    \item{
        Podamos si $q_{ij}$ es imposible de sensar.
        ...
        \begin{pseudo}
            ...
            \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                \For{$i'$ \In $rango$}
                    \State \textbf{break if} $q_{i'j} = \texttt{Pared}$
                    \If{$q_{i'j} = \texttt{Libre}$}
                        \State $R \leftarrow Q$
                        \State $r_{i'j} \leftarrow \texttt{SensorVertical}$
                        \If{$\textsc{Impacta-Otro-Sensor}(r_{i'j}) = \textbf{false}$}
                            \State $podar \leftarrow \textbf{false}$
                        \EndIf
                    \EndIf
                \EndFor
            ...
            \For{$rango$ \In $\{ (j, \ldots, 1), (j, \ldots, n)  \}$}
                \For{$j'$ \In $rango$}
                    \State \textbf{break if} $q_{ij'} = \texttt{Pared}$
                    \If{$q_{ij'} = \texttt{Libre}$}
                        \State $R \leftarrow Q$
                        \State $r_{ij'} \leftarrow \texttt{SensorVertical}$
                        \If{$\textsc{Impacta-Otro-Sensor}(r_{ij'}) = \textbf{false}$}
                            \State $podar \leftarrow \textbf{false}$
                        \EndIf
                    \EndIf
                \EndFor
