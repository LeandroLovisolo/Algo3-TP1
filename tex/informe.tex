...
Representamos un piso $P$ como una matriz de $m$ filas por $n$ columnas, donde cada $p_{ij} \in P$ puede tener uno de los siguientes valores: \texttt{Libre}, \texttt{Sensado}, \texttt{Importante}, \texttt{Pared}, \texttt{SensorVertical}, \texttt{SensorHorizontal}, \texttt{SensorCuadruple}.

Dado un piso $P$ sin sensores, recorremos el árbol de combinaciones de sensores posibles de la siguiente manera:

\begin{enumerate}
    \item{
        \label{buscar-posicion-libre}
        Busco la primera posición libre $(i, j)$ recorriendo el piso de izquierda a derecha y de arriba a abajo, partiendo de la esquina superior izquierda.
        ...
        \begin{pseudo}
            ...
            \While{$i \leq m \land p_{ij} \neq \texttt{Libre}$}
                ...
                \Else
                    \State $i \leftarrow i + 1$, $j \leftarrow 1$
                \EndIf
            \EndWhile
            \State La posición libre hallada es $(i, j)$ si $i \leq m$, o ninguna en caso contrario.
        \end{pseudo}

         Si no se encontró ninguna posición libre, salto al paso \ref{ultimo-paso}.
    }
    \item{
        \label{generar-pisos}
        Genero 4 pisos nuevos:
        \begin{enumerate}
            \item{
                Poniendo un sensor vertical en $(i, j)$.
            }
            \item{
                Poniendo un sensor horizontal en $(i, j)$.
            }
            \item{
                Poniendo un sensor cuádruple en $(i, j)$.
            }
            \item{
                \label{posicion-libre}
                Dejando libre la posición $(i, j)$ (es decir, este nuevo piso es idéntico al piso original.)
            }
        \end{enumerate}
    }
    \item{
        Para cada piso $Q$ generado en el paso anterior, marco como sensadas todas las posiciones libres afectadas por el nuevo sensor (no aplica para el caso \ref{posicion-libre}.)

        \begin{pseudo}
            \If{$q_{ij} \in \{ \texttt{SensorVertical}, \texttt{SensorCuadruple} \}$}
                \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                    \For{$i'$ \In $rango$}
                        \State \textbf{break if} $q_{i'j} = \texttt{Pared}$
                        \State $q_{i'j} \leftarrow \texttt{Sensado}$ \textbf{if} $q_{i'j} = \texttt{Libre}$
                    \EndFor
                \EndFor
            \EndIf
            \If{$q_{ij} \in \{ \texttt{SensorHorizontal}, \texttt{SensorCuadruple} \}$}
                \For{$rango$ \In $\{ (j, \ldots, 1), (j, \ldots, n)  \}$}
                    \For{$j'$ \In $rango$}
                        \State \textbf{break if} $q_{ij'} = \texttt{Pared}$
                        \State $q_{ij'} \leftarrow \texttt{Sensado}$ \textbf{if} $q_{ij'} = \texttt{Libre}$
                    \EndFor
                \EndFor
            \EndIf
        \end{pseudo}
    ...
    \item{
        Para cada piso $Q$ resultante del paso anterior, descarto los que correspondan según el criterio de poda (ver \textit{Podas} más abajo.)
    }
    \item{
        ...
        \begin{enumerate}
            \item{
                Busco la siguiente posición libre $(i', j')$ de la misma manera que en el paso \ref{buscar-posicion-libre}, pero partiendo de $(i, j+1)$ en vez de la esquina superior izquierda, ó $(i+1, 1)$ si $j=n$.
            }
            \item{
                Si se encontró una posición libre, repito los pasos $\ref{generar-pisos}, \ldots, \ref{paso-recursivo}$ utilizando $(i', j')$ en lugar de $(i, j)$.
            }
            \item{
                En caso contrario, decido si el piso es solución.

                \begin{pseudo}
                    \State $esSolucion \leftarrow \textbf{true}$
                    \For{$i$ \In $1, \ldots, m$, $j$ \In $1, \ldots, n$}
                        \If{$q_{ij} = \texttt{Libre}$ $\lor$
                            $(q_{ij} = \texttt{Importante}$ $\land$
                            $\textsc{\#-Sensores-Actuando-Sobre}(q_{ij}) < 2)$}
                            \State $esSolucion \leftarrow \textbf{false}$
                            \State \textbf{break}
                        \EndIf
                    \EndFor
                \end{pseudo}
                ...
                \begin{pseudo}
                    \Procedure{\#-Sensores-Actuando-Sobre}{$q_{ij}$}
                        \State $k \leftarrow 0$
                        \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                            \For{$i'$ \In $rango$}
                                \State \textbf{break if} $q_{i'j} = \texttt{Pared}$
                                \If{$q_{i'j} \in \{ \texttt{SensorVertical}, \texttt{SensorCuadruple} \}$}
                                    \State $k \leftarrow s + 1$
                                    \State \textbf{break}
                                \EndIf
                            \EndFor
                        \EndFor
            ...
            \item{
                Si es solución y es más barata que la mejor solución $S$ hallada hasta el momento, o si aún no se halló ninguna solución, lo marcamos como mejor solución.

                \begin{pseudo}
                    \If{$esSolucion$}
                        \If{$S = \textbf{nil}$ $\lor$ $\textsc{Costo}(Q) < \textsc{Costo}(S)$}
                            \State $S \leftarrow Q$
                        \EndIf
                    \EndIf
                \end{pseudo}
                    ...
                    \Procedure{Costo}{$Q$}
                        ...
                        \For{$i$ \In $1, \ldots, m$, $j$ \In $1, \ldots, n$}
                            \State $c \leftarrow c + 4000$
                                \textbf{if} $q_{ij} \in \{ \texttt{SensorVertical}, \texttt{SensorHorizontal} \}$
                            \State $c \leftarrow c + 6000$
                                \textbf{if} $q_{ij} = \texttt{SensorCuadruple}$
                        \EndFor
                        \State \Return{$c$}                 
                    \EndProcedure
                \end{pseudo}                
    ...
    \item{
        \label{ultimo-paso}
        Devolvemos la mejor solución hallada, o \textbf{nil} en caso de no haberse hallado una.
...
Las podas siempre se realizan sobre los pisos generados en el paso \ref{generar-pisos}.

\begin{itemize}
    \item{
        Si el piso generado posee un nuevo sensor:

        \begin{itemize}
            \item{
                Podamos si su señal impacta otro sensor o viceversa.
            }
            \item{
                Si no es sensor cuádruple, podamos si produce que una posición libre en su misma fila o columna sea imposible de sensar.
            ...
            \item{
                Podamos si el costo del piso supera el de la mejor solución hallada hasta el momento.
            }
        \end{itemize}
    }
    \item{
        Si el piso generado no posee un nuevo sensor, podamos si la posición que dejamos libre es imposible de sensar.
