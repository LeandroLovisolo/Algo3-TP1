...
Representamos el piso como una matriz de $m$ filas $\times$ $n$ columnas, donde cada coordenada puede tener uno de los siguientes valores: \texttt{Libre}, \texttt{Sensado}, \texttt{Importante}, \texttt{Pared}, \texttt{SensorVertical}, \texttt{SensorHorizontal}, \texttt{SensorCuádruple}.

Recorremos el árbol de pisos posibles de la siguiente manera:

\begin{enumerate}
    \item{
        \label{buscar-posicion-libre}
        Busco la primera posición libre recorriendo el piso de izquierda a derecha y de arriba a abajo, partiendo de la esquina superior izquierda. Si no se encontró una, salto al paso \ref{ultimo-paso}.
    ...
    \item{
        ...
        \begin{enumerate}
            \item{
                Poniendo un sensor vertical en esa posición.
            }
            \item{
                Poniendo un sensor horizontal en esa posición.
            }
            \item{
                Poniendo un sensor cuádruple en esa posición.
            }
            \item{
                \label{posicion-libre}
                Dejando esa posición libre (es decir, este nuevo piso es idéntico al piso original.)
            }
        \end{enumerate}
    }
    \item{
        Marco como \textit{sensadas} todas las posiciones afectadas por el nuevo sensor (no aplica para el caso \ref{posicion-libre}.)
    }
    \item{
        Realizo las podas sobre los nuevos pisos (ver \textit{Podas} más abajo.)
    }
    \item{
        \label{paso-recursivo}
        Para cada uno de los 4 pisos que no fue descartado en el paso anterior:
        \begin{enumerate}
            \item{
                Busco la siguiente posición libre luego de la última posición libre hallada, de la misma manera que en el paso \ref{buscar-posicion-libre}.
            }
            \item{
                Si se encontró una posición libre, repito los pasos \ref{generar-pisos}, $\ldots$, \ref{paso-recursivo}.
            }
            \item{
    ...
    \item{
        \label{ultimo-paso}
        Devolvemos la mejor solución hallada, o emitimos un mensaje en caso de no haberse hallado una.
    }
\end{enumerate}
