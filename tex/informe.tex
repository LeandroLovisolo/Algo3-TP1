...
\begin{enumerate}
    ...
    \item{
        ...
        \begin{pseudo}
            \If{$q_{ij} \in \{ \SensorV, \SensorC \}$}
                \For{$i'$ \In $\textsc{Columna-Visible}(q_{ij})$}
                    \State $q_{i'j} \leftarrow \Sensado$ \textbf{if} $q_{i'j} = \Libre$
                \EndFor
            \EndIf
            \If{$q_{ij} \in \{ \SensorH, \SensorC \}$}
                \For{$j'$ \In $\textsc{Fila-Visible}(q_{ij})$}
                    \State $q_{ij'} \leftarrow \Sensado$ \textbf{if} $q_{ij'} = \Libre$
                \EndFor
            \EndIf
        \end{pseudo}

        \begin{pseudo}
            \Procedure{Fila-Visible}{$q_{ij}$}
                \State $C \leftarrow \{ j \}$
                \For{$rango$ \In $\{ (j, \ldots, 1), (j, \ldots, n)  \}$}
                    \For{$j'$ \In $rango$}
                        \State \textbf{break if} $q_{ij'} = \Pared$
        ...
        \begin{pseudo}
            \Procedure{Columna-Visible}{$q_{ij}$}
                \State $F \leftarrow \{ i \}$
                \For{$rango$ \In $\{ (i, \ldots, 1), (i, \ldots, m)  \}$}
                    \For{$i'$ \In $rango$}
                        \State \textbf{break if} $q_{i'j} = \Pared$
                        \State $F \leftarrow F \cup \{ i' \}$
                    \EndFor
                \EndFor
                \State \Return{$F$}
            \EndProcedure
    ...
    \item{
        ...
        \begin{enumerate}
            ...
            \item{
                ...
                \begin{pseudo}
                    \State $esSolucion \leftarrow \True$
                    \For{$i$ \In $1, \ldots, m$, $j$ \In $1, \ldots, n$}
                        \If{$q_{ij} = \Libre$ $\lor$
                            $(q_{ij} = \Importante$ $\land$
                            $\textsc{\#-Sensores-Actuando-Sobre}(q_{ij}) < 2)$}
                            \State $esSolucion \leftarrow \False$
                            \State \textbf{break}
                        \EndIf
                    \EndFor
                \end{pseudo}
                    ...
                    \Procedure{Costo}{$Q$}
                        ...
                        \For{$i$ \In $1, \ldots, m$, $j$ \In $1, \ldots, n$}
                            \State $c \leftarrow c + 4000$
                                \textbf{if} $q_{ij} \in \{ \SensorV, \SensorH \}$
                            \State $c \leftarrow c + 6000$
                                \textbf{if} $q_{ij} = \SensorC$
                        \EndFor
                        \State \Return{$c$}                 
                    \EndProcedure
                \end{pseudo}                
...
\begin{itemize}
    \item{
        Podamos si su señal impacta otro sensor o viceversa; es decir, si $\textsc{Impacta-Otro-Sensor}(q_{ij}) = \True$.
        ...
        \begin{pseudo}
            \Procedure{Sensa-Verticalmente}{$q_{ij}$}
                \State \Return{$q_{ij} \in \{ \SensorV, \SensorC \}$}
            \EndProcedure
        \end{pseudo}

        \begin{pseudo}
            \Procedure{Sensa-Horizontalmente}{$q_{ij}$}
                \State \Return{$q_{ij} \in \{ \SensorH, \SensorC \}$}
            \EndProcedure
        \end{pseudo}
    }
    \item{
        ...
        \begin{pseudo}
            \State $podar \leftarrow \False$
            \If{$q_{ij} = \SensorV$}
                \For{$j'$ \In $\textsc{Fila-Visible}(q_{ij})$}
                    \If{$q_{ij'} = \Libre \land \textsc{Hay-Un-Sensor-Horizontal-En-Su-Columna}(q_{ij'})$}
                        \State $podar \leftarrow \True$
                        \State \textbf{break}
                    \EndIf
                \EndFor
            \EndIf
            \If{$q_{ij} = \SensorH$}
                \For{$i'$ \In $\textsc{Columna-Visible}(q_{ij})$}
                    \If{$q_{i'j} = \Libre \land \textsc{Hay-Un-Sensor-Vertical-En-Su-Fila}(q_{i'j})$}
                        \State $podar \leftarrow \True$
                        \State \textbf{break}
                    \EndIf
                \EndFor
    ...
    \item{
        ...
        \begin{pseudo}
            ...
            \For{$i'$ \In $\textsc{Columna-Visible}(q_{ij})$}
                \If{$q_{i'j} = \Libre$}
                    \State $R \leftarrow Q$
                    \State $r_{i'j} \leftarrow \SensorV$
                    \If{$\textsc{Impacta-Otro-Sensor}(r_{i'j}) = \False$}
                        \State $podar \leftarrow \False$
                    \EndIf
                \EndIf
            \EndFor
            \For{$j'$ \In $\textsc{Fila-Visible}(q_{ij})$}
                \If{$q_{ij'} = \Libre$}
                    \State $R \leftarrow Q$
                    \State $r_{ij'} \leftarrow \SensorV$
                    \If{$\textsc{Impacta-Otro-Sensor}(r_{ij'}) = \False$}
                        \State $podar \leftarrow \False$
                    \EndIf
                \EndIf
            \EndFor
